...
// Accent color hex map for favicon regeneration
const ACCENT_COLORS = {
...
function updateDynamicFavicon() {
    try {
        const customLogo = localStorage.getItem('as_systems_logo_url');
        if (customLogo) return; // Si hay logo personalizado, no tocar el favicon

        const accent = localStorage.getItem('as_accent') || 'indigo';
        const theme  = localStorage.getItem('as_theme')  || 'dark';
        const colors = ACCENT_COLORS[accent] || ACCENT_COLORS.indigo;
        const bg     = theme === 'dark' ? '%230f172a' : '%23f1f5f9';
        const fromC  = colors.from.replace('#', '%23');
        const toC    = colors.to.replace('#', '%23');

        const svg = `data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20viewBox%3D%220%200%20100%20100%22%3E%3Crect%20width%3D%22100%22%20height%3D%22100%22%20rx%3D%2222%22%20fill%3D%22${bg}%22%2F%3E%3Cdefs%3E%3ClinearGradient%20id%3D%22g%22%20x1%3D%220%25%22%20y1%3D%220%25%22%20x2%3D%22100%25%22%20y2%3D%22100%25%22%3E%3Cstop%20offset%3D%220%25%22%20stop-color%3D%22${fromC}%22%2F%3E%3Cstop%20offset%3D%22100%25%22%20stop-color%3D%22${toC}%22%2F%3E%3C%2FlinearGradient%3E%3C%2Fdefs%3E%3Ctext%20x%3D%2250%22%20y%3D%2268%22%20font-size%3D%2252%22%20fill%3D%22url(%23g)%22%20font-family%3D%22'Outfit'%2C'Inter'%2Csans-serif%22%20text-anchor%3D%22middle%22%20font-weight%3D%22900%22%3EAS%3C%2Ftext%3E%3C%2Fsvg%3E`;
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.accent-option').forEach(opt => {
        opt.addEventListener('click', (e) => {
            ...
            updateDynamicFavicon(); // Actualizar favicon con nuevo acento
        });
    });
